...
def confirm_eligibility(identity):
    '''
    Confirm that a particular identity is eligible to vote.
...
def appointment_schedule(identity, blinded_key, appointment_selection):
    ...
    selected appointment is available. If both are true, tell the voter
...
def sign_key(identity, date, registrar_key):
    '''
    Record the identity and date, and sign the voter's blinded key (stored
